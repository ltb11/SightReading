...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=120mm]{./assets/projection.png}
    \caption{Illustration of the horizontal projection (right) of a stave (left)}
    \label{projection}
\end{figure}
...
An additional problem we found was that on pages with 5 or more staves, the algorithm could incorrectly detect a set of 5 equally spaced lines from different staves. For example, the first line of each of the 5 staves might be recognised as a stave itself. We solved this trivially by specifying a maximum spacing between stave lines (see image \ref{broken_staves}).

\begin{figure}[ht!]
    \centering
    \includegraphics[width=80mm]{./assets/BrokenStaves.jpg}
    \caption{Stave detection before the correction}
    \label{broken_staves}
\end{figure}
